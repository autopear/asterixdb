...
public class LeveledLSMHarness implements ILSMHarness {
    ...
    public LeveledLSMHarness(ILSMIndex lsmIndex, ILSMIOOperationScheduler ioScheduler, ILSMMergePolicy mergePolicy,
        ...
        try {
            synchronized (opTracker) {
                try {
                    ...
                    exitOperationalComponents(ctx, opType, failedOperation);
                    ctx.setAccessingComponents(false);
                    exitLeveledOperation(ctx, opType, newComponents, failedOperation);
                } catch (Throwable e) { // NOSONAR: Log and re-throw
                    LOGGER.warn("Failure exiting components", e);
                    throw e;
                } finally {
    ...
    private void exitLeveledOperation(ILSMIndexOperationContext ctx, LSMOperationType opType,
            List<ILSMDiskComponent> newComponents, boolean failedOperation) throws HyracksDataException {
        // Then, perform any action that is needed to be taken based on the operation type.
        switch (opType) {
            case FLUSH:
                // newComponent is null if the flush op. was not performed.
                if (!failedOperation && newComponents != null && !newComponents.isEmpty()) {
    ...
    private void exitLeveledComponents(ILSMIndexOperationContext ctx, LSMOperationType opType,
            List<ILSMDiskComponent> newComponents, boolean failedOperation) throws HyracksDataException {
        long before = 0L;
        if (ctx.isTracingEnabled()) {
            before = System.nanoTime();
    ...
    public void merge(ILSMIOOperation operation) throws HyracksDataException {
        ...
        try {
            try {
                operation.getCallback().beforeOperation(operation);
                List<ILSMDiskComponent> newComponents = lsmIndex.leveledMerge(operation);
            ...
            exitLeveledComponents(operation.getAccessor().getOpContext(), LSMOperationType.MERGE,
                    operation.getNewComponents(), operation.getStatus() == LSMIOOperationStatus.FAILURE);
    ...
    @Override
    public ILSMIOOperation scheduleLeveledMerge(ILSMIndexOperationContext ctx) throws HyracksDataException {
        ILSMIOOperation operation;
        synchronized (opTracker) {
            operation = lsmIndex.createLeveledMergeOperation(ctx);
        }
        ioScheduler.scheduleOperation(operation);
        return operation;
    }
