...
public class ConcurrentMergePolicy extends StackMergePolicy {
    /**
    ...
    public List<ILSMDiskComponent> getMergableComponents(List<ILSMDiskComponent> components) {
        ...
        int count = 0;
        for (; count < numComponents; count++) {
            if (components.get(count).getState() != ComponentState.READABLE_UNWRITABLE) {
                break;
            }
        }
        if (count < minMergeComponentCount) {
        ...
        long[] componentSizeSum = new long[count];
        for (int i = 0; i < count; i++) {
            componentSizeSum[i] = components.get(i).getComponentSize() + (i > 0 ? componentSizeSum[i - 1] : 0);
        }
