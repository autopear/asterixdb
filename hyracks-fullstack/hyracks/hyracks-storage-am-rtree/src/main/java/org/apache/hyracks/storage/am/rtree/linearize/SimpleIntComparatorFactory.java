...
public class SimpleIntComparatorFactory implements ILinearizeComparatorFactory {
    ...
    public static SimpleIntComparatorFactory get(int dim) {
        return new SimpleIntComparatorFactory(dim);
    }

    public SimpleIntComparatorFactory(int dim) {
        this.dim = dim;
    ...
    public ILinearizeComparator createBinaryComparator() {
        return new SimpleIntComparator(dim);
    }
    ...
    public String byteToString(byte[] b, int s, int l) {
        if (b == null || b.length == 0 || l == 0 || s >= b.length) {
            ...
            StringBuilder sb = new StringBuilder("[" + IntegerPointable.getInteger(b, s));
            for (int i = 1; i < dim; i++) {
                sb.append("," + IntegerPointable.getInteger(b, s + (i * l)));
            }
            sb.append("]");
            return sb.toString();
        }
